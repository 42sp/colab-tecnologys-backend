...
export const access = (app: Application) => {
	...
	app.service(accessPath).hooks({
		...
		before: {
			all: [
				schemaHooks.validateQuery(accessQueryValidator),
				schemaHooks.resolveQuery(accessQueryResolver),
			],
			find: [],
			get: [],
			create: [
				schemaHooks.validateData(accessDataValidator),
				schemaHooks.resolveData(accessDataResolver),
			],
			patch: [
				schemaHooks.validateData(accessPatchValidator),
				schemaHooks.resolveData(accessPatchResolver),
			],
			remove: [],
		},
		after: {
